...
const BUTTONS = {
  ...
  config: {
    ...
    noBorder: true,
    action: () => {
      // abre la sidebar de configuración
      if (typeof browser !== 'undefined' && browser.sidebarAction) {
        browser.sidebarAction.open();
      } else {
        console.log("Sidebar solo funciona en Firefox Extension Context");
      }
    },
  },
};
...
function loadSettingsFromStorage() {
  // Usamos localStorage estándar para compartir con la sidebar
  const saved = localStorage.getItem('timerSettings_v2');
  if (saved) {
    try {
      const data = JSON.parse(saved);
      workDurationSettings = data.work;
      restDurationSettings = data.rest;
      
      // Recalcular segundos
      WORK_SECONDS = workDurationSettings * 60;
      REST_SECONDS = restDurationSettings * 60;
    } catch (e) {
...
function draw() {
  ...
  if (remainingSeconds < 0) {
    ...
    text(`${fmtMMSS(excededSeconds)}`, width - 240, 355);
  }
}
